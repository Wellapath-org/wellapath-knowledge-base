...
def build_package():
    ...
    decisions = [
        ...
        _decision(
            ...
            % (scoring_affecting, scoring_delta["conditions_touched"]),
            {
                ...
                "score_urgency_ranking_delta": {
                    "status": "MEASURED with the shipped Mobile engine",
                    "source": "reports/im003_mobile_measurement_v1.json",
                    "mobile_pull_request": 76,
                    "mobile_head": "13be0d4937b1c49d6a49ddf096c5d5b6a47c2091",
                    "scenarios_measured": 63,
                    "red_flag_changes": 0,
                    "urgency_changes": 25,
                    "urgency_escalations": 24,
                    "urgency_de_escalations": 1,
                    "top_condition_changes_overlapping": 31,
                    "every_changed_top_condition_became": "malaria",
                    "open_safety_blocker": "IM003-SB-001",
                },
                "open_safety_blocker": {
                    "blocker_id": "IM003-SB-001",
                    "status": "open_awaiting_clinical_and_product_adjudication",
                    "summary": "S10_path_limit_pressure de-escalated urgency from emergency to urgent with no red-flag change, because additive tokens re-ranked lassa_fever (26, emergency) below malaria (25 -> 52, urgent).",
                    "registry": "reports/im003_safety_blockers_v1.json",
                },
            },
            "56 pairs; %d newly reachable scoring tokens" % scoring_affecting,
            "CHANGES SCORING INPUT, and the shipped-engine measurement is now "
            "in: 15 newly reachable tokens touching 31 of 50 conditions; 63 "
            "scenarios measured; 25 urgency changes, of which 24 escalate and "
            "ONE DE-ESCALATES emergency to urgent with no red-flag change "
            "(IM003-SB-001). Red-flag invariance did not imply urgency "
            "invariance. This decision cannot be taken while that blocker is "
            "open, and no IM-003 activation is permitted meanwhile.",
    ...
    return {
        ...
        "decomposition_recommendation": {
            ...
            "status": "NARROWED_PENDING_IM003_SB_001",
            "narrowing_note": (
                "Recorded at I2/W3 Step 8. The recommendation below was written "
                "BEFORE the shipped-engine measurement existed, and its "
                "'then C separately' clause could be read as permitting "
                "scoring-affecting re-branching once C was reviewed. That "
                "reading is now suspended: IM003-SB-001 is open, and no subset "
                "— inert or otherwise — may be implemented or activated while "
                "it is. The original text is retained verbatim below rather "
                "than edited, so the reasoning that led here stays auditable."
            ),
            "suspended_clause": "then C separately",
            "suspended_because": "IM003-SB-001 is open_awaiting_clinical_and_product_adjudication",
            "still_an_engineering_recommendation_only": True,
